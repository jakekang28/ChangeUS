...
export default function MainPage() { //로그인 시 
  ...
  const [seedCnt, setSeedCnt] = useState(0)
  const [totSeedCnt, setTotalSeedCnt] = useState(0)
  
  useEffect(() => {
    axios.get(('http://localhost:4003/api/users/userscore')).then((res) =>{
      
      setSeedCnt(res.data[0].score)
  })
    console.log("테스트");
    const socket = new WebSocket("ws://localhost:4003"); // 서버에 연결
    ...
    socket.addEventListener("message", (message) => {
      const data = JSON.parse(message.data)[0]['SUM(score)']
      setTotalSeedCnt(data)
      console.log("Received from server:", totSeedCnt);
    });
  ...
  return (
    <Container $backgroundImage={backgroundImage}>
      ...
      <ContentContainer>
        <SeedContainer seedCnt={seedSort === 'my' ? seedCnt : totSeedCnt} />
        <CharacterImage
      ...
      <ButtonsContainer>
        ...
        <NavButton selection="challenge" onClick={() => navigate("/challenges-dashboard")}>챌린지</NavButton>
      </ButtonsContainer>
    </Container>
  );
}
